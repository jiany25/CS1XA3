...
identifier =  do {x <- try alphaNum;
                  return (x:"") <|> return ""} -- #TODO replace me
...
item = do { ss <- identifier;
            symbol ":";
            d <- integer;
            return (ss,d)} -- #TODO replace me
...
items p = do { is <- sepBy item (symbol ",");
               return is } -- #TODO replace me
